...
export default async function AsiakasPage({ params }: Params) {
  ...
  // Hae asiakas — varmista että kuuluu samaan organisaatioon
  ...
  const { data: tapahtumat } = await supabase
    .from('tapahtumat')
    .select('id, tyyppi, kuvaus, paivamaara, summa_alv0, alv_prosentti, verovuosi')
    .eq('asiakas_id', params.id)
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#1c2b1e', color: '#f0f4f1' }}>
      ...
      {/* Sidebar */}
      <nav style={{
        width: '220px', flexShrink: 0, backgroundColor: '#162318',
        borderRight: '1px solid #2e4a32', display: 'flex',
        flexDirection: 'column', padding: '1.5rem 0',
      }}>
        <a href="/" style={{
          fontFamily: "'Fraunces', serif", fontSize: '1.2rem', fontWeight: 600,
          color: '#1D9E75', textDecoration: 'none', padding: '0 1.25rem',
        ...
        <div style={{ marginTop: 'auto', padding: '1rem 1.25rem', borderTop: '1px solid #2e4a32' }}>
          <p style={{ fontSize: '0.8rem', color: '#7a9e7e', margin: '0 0 0.5rem' }}>
      ...
      <main style={{ flex: 1, padding: '2.5rem 2rem', overflowY: 'auto' }}>
        <div style={{ maxWidth: '880px' }}>
          ...
          <p style={{ fontSize: '0.85rem', color: '#7a9e7e', margin: '0 0 1.5rem' }}>
            <a href="/asiakkaat" style={{ color: '#1D9E75', textDecoration: 'none' }}>Asiakkaat</a>
            {' / '}{nimi}
          </p>

          {/* Otsikko + muokkausnappi */}
          ...
          {/* Perustiedot */}
          <div style={{
            border: '1px solid #2e4a32', borderRadius: '0.75rem',
          ...
          {/* Metsätilat */}
          <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: '1.2rem', fontWeight: 300, color: '#e8f0e9', margin: '0 0 1rem' }}>
            Metsätilat
          </h2>
                ...
                <tbody>
                  {metsatilat.map((t, i) => (
                    <tr key={t.id} style={{ borderBottom: i < metsatilat.length - 1 ? '1px solid #2e4a32' : 'none' }}>
                      ...
                      <td style={{ padding: '0.85rem 1rem', color: '#9ab89e' }}>
                        {t.hankintahinta ? `${Number(t.hankintahinta).toLocaleString('fi-FI')} €` : '—'}
                      </td>
                    </tr>
                  ))}
                </tbody>
                  ...
                  {tapahtumat.map((t, i) => (
                    <tr key={t.id} style={{ borderBottom: i < tapahtumat.length - 1 ? '1px solid #2e4a32' : 'none' }}>
                      <td style={{ padding: '0.85rem 1rem', color: '#9ab89e', fontSize: '0.9rem' }}>
                        {new Date(t.paivamaara).toLocaleDateString('fi-FI')}
                      </td>
                      <td style={{ padding: '0.85rem 1rem' }}>
                        <span style={{
                          padding: '0.2rem 0.6rem', borderRadius: '99px', fontSize: '0.75rem', fontWeight: 500,
                          background: t.tyyppi === 'tulo' ? '#1a3d2b' : '#3d1a1a',
                          color: t.tyyppi === 'tulo' ? '#1D9E75' : '#e87171',
                        }}>
